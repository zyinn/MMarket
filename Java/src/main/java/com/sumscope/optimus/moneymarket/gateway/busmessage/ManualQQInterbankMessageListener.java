 ...
 * * 旧版本QQ同业理财信息监听器，监听器本身不处理业务逻辑，仅将数据转交给业务服务进行处理
 */
@Component
public class ManualQQInterbankMessageListener extends AbstractQQMsgListener {
    ...
    protected void processQQJmsMessage(Map<String, Object> map) {
        try {
            ...
            LogManager.warn("旧版QQ同业理财接入数据失败。"+map.toString());
        }
    }
    ...
    private MmQuote convertQQInterbankMsgToQuote(Map<String, Object> qqMsg) {
        return qqMessageToQuoteConverter.convertManualQQInterbankMsgToQuote(qqMsg);
    }
}
